...
struct MasterServer {
    ...
    MasterServer() {}
    MasterServer(const ServerId *id) : pid(id->pid()), hostname(id->hostname()), port(id->port()) {}
...
class RoutingServiceImpl final : public RoutingService::Service
{
    Status GetAvailableServer(ServerContext *context, const Empty *empty, ServerId *id) override
    {
        ...
        id->set_pid(available_server.pid);
        id->set_hostname(available_server.hostname);
        id->set_port(available_server.port);
        return Status::OK;
    }
    ...
    Status AddServer(ServerContext *context, const ServerId *id, Empty *empty) override
    {
        ...
        /*
        cout << "List of MasterServers:\n";
        for (auto server : servers) {
            cout << "\t" << server.pid << ", " + server.hostname + ":" + server.port + "\n";
        }
        cout << "Available Server:\n";
        cout << "\t" << available_server.pid << ", " + available_server.hostname + ":" + available_server.port + "\n";*/
